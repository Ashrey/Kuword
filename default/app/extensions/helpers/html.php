...
class Html
{
    ...
    {
        $code = '';
        foreach(Tag::getJs() as $js) {
			if(isset($js['partial'])){
				$code .= '<script type="text/javascript">'.$js['partial'].'</script>';
			}else{
				$code .= '<script type="text/javascript" src="' . PUBLIC_PATH .  "javascript/{$js['src']}.js\"></script>" . PHP_EOL;
			}
        }
        return $code;
    }
